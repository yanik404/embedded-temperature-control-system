...
1. **Aufbau:** hochwertige, sofort sichtbare Produktvisualisierung auf Basis der gelieferten STEP-Geometrie. **AUSSEN** zeigt das glaubwürdig fertig montierte Gerät unverändert mit mattem Gehäuse, tief eingesetztem Becher, Peltierkontakten auf Becherhöhe sowie OLED und vier physischen Tastern in der unteren Frontaussparung. **INNEN** öffnet dieselbe Hülle kontrolliert. Peltier links/rechts besitzen keramische Körper und gebürstete Metallkontaktflächen; TMP36 #1 ist als seitliches TO-92-Bauteil erkennbar, TMP36 #2 sitzt auf einem sichtbaren Federkontakt mittig unter dem Becher. Genau ein horizontaler Lüfter liegt darunter. Im unteren Elektronikraum ist das aus `PCB_TOP_original_3D.png` abgeleitete originale 80-mm-PCB mit aufgesetztem Pico W sichtbar. Der Cup Detector ist als mechanischer Seitenschalter mit langem flachem Taststößel modelliert, der vom eingesetzten Becher gedrückt wird; auch der untere optische Lichtsensor besitzt eine eigene physische Bauform. Jede dieser Bauformen ist eine separate Ebene und lässt sich über den zugehörigen Eintrag mit kleinem Formsymbol exakt fokussieren. Auf Mobilgeräten stehen Produkt und Legende untereinander, ohne Callout-Linien. Die Visualisierung benötigt weder WebGL noch externe Assets.
...
- `ui-v5/src/pcb-top-original.webp` – kompakte transparente WebP-Ableitung der gelieferten echten `PCB_TOP_original_3D.png` für die sichtbare 80-mm-PCB-Ebene
...
- `ui-v5/src/product-v2-cutaway.svg` – Einbettungsrahmen mit getrennten realen Bauteilebenen und am Placement Guide ausgerichteten Hotspots

Die stabilisierte Produktion bettet Außenansicht, Cutaway-Basis und die transparente Original-PCB-Ebene als Data-URLs in die bestehende SVG-/DOM-/Canvas-Architektur ein. Alle kleineren Hardwareteile bleiben kompakte SVG-Ebenen. Dadurch bleibt das Dashboard eine einzelne Flash-Datei ohne zusätzliche HTTP-Asset-Requests. Frühere WebGL-, X-Ray-, Scroll-Mode- und Bedienlabor-Prototypen wurden nach Abschluss der Technologieentscheidung entfernt. Die aktuelle Architektur steht in `docs/ui-v3-architecture.md` und `docs/ui-v4-architecture.md`.
...
`powershell -ExecutionPolicy Bypass -File tools/capture_ui_review.ps1 -Round manual` rendert die sieben Referenz-Viewports automatisiert mit exakt gesetzten Chrome-/Edge-DevTools-Gerätemetriken nach `build/ui-review/manual/`. `python tools/audit_ui_layout.py` prüft AUFBAU, REGELKREIS und LIVE bei denselben sieben Viewports auf sichtbare Kollisionen und horizontalen Overflow. Mit `tools/capture_ui_review.py --product-test product|scale20|grayscale|blur|silhouette` lässt sich die Produktgrafik reproduzierbar isolieren. `tests/test_v3_visual_contract.py` fixiert die SHA-256-Hashes der aktiven Produkt-Assets und der erhaltenen STEP-Fallbacks; `tests/test_product_v2_responsive.py` prüft AUSSEN und INNEN automatisiert bei 1920, 1440, 1366, 768 und 390 Pixeln.
