    ...
    try:
        ...
        expression = """(()=>{
          ...
          return {delta:m.deltaTemperature(status),current:m.totalCurrent(status),power:m.electricalPower(status),
            ...
            eta:m.estimateEtaMinutes(status,2),invalidDelta:m.deltaTemperature({...status,temp2_ok:false})};
        })()"""
...
print("dashboard insight checks passed: delta-T, current, power, dT/dt, ETA, disconnect and recovery")
