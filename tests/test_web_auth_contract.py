...
assert source.count('if (!request_authorized(text)) return send_unauthorized') == 2
...
stop_handler = source[source.index('POST /api/stop '):source.index('static const char setpoint_prefix')]
