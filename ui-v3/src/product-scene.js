...
var model={state:"AUS",power:0,temperature:22,fan:0,mode:"product",lens:"product",focus:"system",explode:0,exploded:false,lowMotion:false,frameInterval:33,pointerX:0,pointerY:0,visible:true,partsA:[1,1,1,1],partsB:[1,1,1,1]};
var locations={};
...
vec2 map(vec3 p){
 ...
 float plateY=-.09-ex*.16;if(partsB.w>.5){z=join(z,vec2(cylY(p-vec3(0.,plateY,0.),.12,1.25),4.));
 z=join(z,vec2(torusY(p-vec3(0.,plateY+.09,0.),vec2(1.17,.025)),13.));}
 vec3 pel1=p-vec3(-.51,-.32-ex*.50,0.);vec3 pel2=p-vec3(.51,-.32-ex*.50,0.);
 if(partsA.z>.5)z=join(z,vec2(box(pel1,vec3(.47,.11,.68)),5.));if(partsA.w>.5)z=join(z,vec2(box(pel2,vec3(.47,.11,.68)),17.));
 ...
 vec3 fp=p-vec3(0.,-.98-ex*1.15,.82+ex*.18);if(partsB.x>.5){z=join(z,vec2(torusZ(fp,vec2(.47,.065)),7.));z=join(z,vec2(cylZ(fp,.075,.105),7.));
 ...
 vec3 s1=p-vec3(1.00+ex*.32,.34+ex*.28,.12);if(partsA.x>.5){z=join(z,vec2(box(s1,vec3(.085,.13,.06)),8.));z=join(z,vec2(box(s1-vec3(-.05,-.20,0.),vec3(.018,.10,.018)),16.));z=join(z,vec2(box(s1-vec3(.05,-.20,0.),vec3(.018,.10,.018)),16.));}
 vec3 s2=p-vec3(-.78-ex*.30,1.10+ex*.62,.64);if(partsA.y>.5){z=join(z,vec2(box(s2,vec3(.075,.12,.055)),9.));z=join(z,vec2(box(s2-vec3(0.,-.20,-.05),vec3(.02,.09,.02)),16.));}
 vec3 pcb=p-vec3(-1.42-ex*.72,-.33-ex*.20,.08);if(partsB.y>.5){z=join(z,vec2(box(pcb,vec3(.46,.39,.035)),10.));z=join(z,vec2(box(pcb-vec3(.39,.12,-.08),vec3(.10,.33,.10)),15.));
 ...
 vec3 console=p-vec3(1.38+ex*.58,-.24-ex*.12,.13);if(partsB.z>.5){z=join(z,vec2(box(console,vec3(.36,.36,.055)),19.));z=join(z,vec2(box(console-vec3(0.,.12,.075),vec3(.24,.105,.025)),20.));
 for(int i=0;i<4;i++){z=join(z,vec2(cylZ(console-vec3((float(i)-1.5)*.135,-.14,.08),.035,.04),21.));}
...
function setMode(mode){model.mode=mode||"product";}function setLens(lens){model.lens=lens||"product";}function setFocus(focus){model.focus=focus||"system";}function setExploded(value){model.exploded=!!value;}function setComponents(ids){var set=new Set(ids||[]),any=function(names){return names.some(function(id){return set.has(id);})?1:0;};model.partsA=[any(["temp1"]),any(["temp2"]),any(["peltier1"]),any(["peltier2"])];model.partsB=[any(["fan"]),any(["pico","adc","current1","current2","power","wifi","webserver"]),any(["oled","buttons","rgb","leds","light"]),any(["plate"])];}function setLowMotion(value){model.lowMotion=!!value;}function setFrameRate(fps){model.frameInterval=1000/Math.max(1,Number(fps)||30);}function setProjectionListener(callback){projectionCallback=callback;if(callback&&canvas)requestAnimationFrame(function(){callback(project);});}
window.ProductScene={initialise:initialise,update:update,setMode:setMode,setLens:setLens,setFocus:setFocus,setExploded:setExploded,setComponents:setComponents,setLowMotion:setLowMotion,setFrameRate:setFrameRate,setProjectionListener:setProjectionListener,project:project,resize:resize};
})();
