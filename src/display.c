...
static uint8_t display_address = SSD1306_ADDRESS;
static bool sh1106_mode;
...
static void command(uint8_t value) {
    ...
    i2c_write_blocking(I2C_PORT, display_address, bytes, sizeof(bytes), false);
}
...
static void flush(void) {
    ...
    for (uint8_t page = 0; page < 8u; ++page) {
        ...
        /* SH1106 RAM is 132 columns wide; a 128-pixel panel starts at column 2. */
        command(sh1106_mode ? 0x02u : 0x00u);
        command(0x10u);
        memcpy(&packet[1], &framebuffer[page * OLED_WIDTH], OLED_WIDTH);
        i2c_write_blocking(I2C_PORT, display_address, packet, sizeof(packet), false);
    }
}

bool display_init(void) {
    const uint8_t probe = 0x00u;
    const uint8_t candidates[] = {SSD1306_ADDRESS, 0x3Du};
    available = false;
    /* Some OLED modules need a few hundred milliseconds after board power-up
       before their controller acknowledges I2C. */
    for (uint8_t attempt = 0; attempt < 40u && !available; ++attempt) {
        for (size_t i = 0; i < sizeof(candidates); ++i) {
            if (i2c_write_blocking(I2C_PORT, candidates[i], &probe, 1, false) == 1) {
                display_address = candidates[i];
                available = true;
                break;
            }
        }
        if (!available) sleep_ms(25u);
    }
    if (!available) return false;
    sh1106_mode = display_address == 0x3Du;
    if (sh1106_mode) {
        const uint8_t init[] = {0xAE,0xD5,0x80,0xA8,0x3F,0xD3,0x00,0x40,0xAD,0x8B,
